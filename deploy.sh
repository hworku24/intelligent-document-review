...
MIGRATION_COMMAND=$(aws cloudformation describe-stacks --stack-name VeraStack --query "Stacks[0].Outputs[?OutputKey=='DeployMigrationCommand'].OutputValue" --output text)
...
FRONTEND_URL=$(aws cloudformation describe-stacks --stack-name VeraStack --query "Stacks[0].Outputs[?OutputKey=='FrontendURL'].OutputValue" --output text)
API_URL=$(aws cloudformation describe-stacks --stack-name VeraStack --query "Stacks[0].Outputs[?OutputKey=='ApiUrl'].OutputValue" --output text)
